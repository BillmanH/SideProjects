...
colnamespath <- "/Users/colinrogers/modeling/ColNames.txt"

#modelFile <- "C:/Users/Bill/data.csv"
...
Functions
'''
create_new_model <- function(model_name,stoplist="StopList_en.txt",N=25,remove_twitter=F,docType="*.csv", mac=F){
  dir.create(model_name)
  if (mac==T) {model_file <- file.choose()} #adjust if mac OS as chose.file doesn't work on macs
  
  else {model_file <- choose.filechooses(default = "", caption = "Select the file that will train your model",
                   multi = F, filters = docType,
                   index = nrow(Filters)) }
  mySep <- ","
  if (docType=="*.txt"){mySep <- "\t"} #change the seperator if .txt
  
  doc1 <-read.csv(model_file, #read in the datafile
                  header = TRUE,
                  quote = "",
                  sep = mySep)
  print(dim(doc1))
  ...
  if(remove_twitter==T){
    documents <- subset(doc1, doc1$pageType!="twitter")
  ...
  textrows<-gsub("[^[:alnum:]]", " ",documents$full.Text)
  mallet.instances <- mallet.import(as.character(documents$id), as.character(textrows), stoplist ,token.regexp = "\\p{L}[\\p{L}\\p{P}]+\\p{L}")
  set.seed(2015)
  ...
  print("Printing ....")
  write.csv(doc.topics.out, file=paste(model_name,"/","doc_topics",Sys.Date(),".csv", sep=""))
  write.csv(topic.words.final, file=paste(model_name,"/","topic_words",Sys.Date(),".csv", sep=""))
